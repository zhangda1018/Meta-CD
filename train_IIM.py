...
from utils.utils import IIM_accuracy as accuracy
from utils.utils import AverageMeter
import sys
#sys.path.append('/data1/wfy/CrowdStudy/SAM_CD_newroot/SAM-CD')
###################### Data and Model ########################
from models.SAM_CD_IIM import SAM_CD as Net
#from models.SAM_CD import SAM_CD as Net
NET_NAME = 'MetaCD'

# from datasets import WHU_CD as RS
from datasets import S2Looking as RS
DATA_NAME = 'S2Looking'
THIS_NAME = 'S2Looking_5_MetaCD'
...
args = {
    ...
    'chkpt_dir': os.path.join(working_path, 'checkpoints', THIS_NAME),
...
def main():
    ...
    sam_1_params = list(net.sam_1.parameters())
    binar_params = list(net.Binar.parameters())
    optimizer = optim.SGD([{"params":filter(lambda p: p.requires_grad, sam_1_params), "lr":args['lr'],
                          },{"params":binar_params,"lr":args['binar_lr']}],weight_decay=args['weight_decay'], momentum=args['momentum'], nesterov=True)
    # optimizer1 = optim.SGD(filter(lambda p: p.requires_grad, sam_1_params), args['lr'],
    ...
    #                       weight_decay=args['weight_decay'], momentum=args['momentum'], nesterov=True)
    #optimizer2 = optim.Adam(binar_params, lr=args['binar_lr'])
...
def train(train_loader, net, optimizer, val_loader):
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            thres = threshold_map.cpu().detach().numpy()
            # pdb.set_trace()
            acc_curr_meter = AverageMeter()
            for (pred, label, threshold) in zip(preds, labels, thres):
                acc, precision, recall, F1, IoU = accuracy(pred, label, threshold)
...
def validate(val_loader, net, curr_epoch):
    ...
    for vi, data in enumerate(val_loader):
        ...
        with torch.no_grad():
            outputs, outA, outB, _, threshold_map= net(imgs_A, imgs_B)
        ...
        labels = labels.cpu().detach().numpy()
        preds = F.sigmoid(outputs).numpy()
        threshold_map = threshold_map.cpu().detach().numpy()
        for (pred, label, threshold) in zip(preds, labels, threshold_map):
            acc, precision, recall, F1, IoU = accuracy(pred, label, threshold)
            F1_meter.update(F1)
...
if __name__ == '__main__':
    main()
# CUDA_VISIBLE_DEVICES=2 python train_IIM.py